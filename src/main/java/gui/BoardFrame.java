...
public class BoardFrame extends javax.swing.JFrame {
	...
	/**
	 * Creates a new board frame with the input design file imbedded into a host cad
	 * software.
	 *
	 ...
	 */
	public static BoardFrame get_embedded_instance(String p_design_file_path_name, BoardObservers p_observers,
			IdNoGenerator p_id_no_generator, java.util.Locale p_locale) {
	 ...
	 * created from a start menu frame. If p_option = SINGLE_FRAME, this frame is
	 * created directly a single frame. If p_option = Option.IN_SAND_BOX, no
	 * security sensitive actions like for example choosing If p_option =
	 * Option.WEBSTART, the application has been started with Java Webstart. files
	 * are allowed, so that the frame can be used in an applet. Currently
	 * Option.EXTENDED_TOOL_BAR is used only if a new board is created by the
	 * application from scratch. If {@code p_test_level > RELEASE_VERSION},
	 * functionality not yet ready for release is included. Also the warning output
	 ...
	 */
	public BoardFrame(DesignFile p_design, Option p_option, TestLevel p_test_level, java.util.Locale p_locale,
	 ...
	 * p_item_id_no_generator are used for syncronizing purposes, if the frame is
	...
	BoardFrame(DesignFile p_design, Option p_option, TestLevel p_test_level, BoardObservers p_observers,
		...
		this.screen_messages = new ScreenMessages(this.message_panel.status_message, this.message_panel.add_message,
	...
	/**
	 * Reads an existing board design from file. If p_is_import, the design is read
	 * from a scpecctra dsn file. Returns false, if the file is invalid.
	 */
	...
	/**
	 * Saves the interactive settings and the design file to disk. Returns false, if
	 * the save failed.
	 */
	boolean save() {
	...
	/**
	 * Sets contexts sensitive help for the input component, if the help system is
	 * used.
	 *
