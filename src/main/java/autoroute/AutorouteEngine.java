...
public class AutorouteEngine {
	private static final Logger LOGGER = LoggerFactory.getLogger(AutorouteEngine.class);

	/**
	 * Creates a new instance of BoardAoutorouteEngine If p_maintain_database, the
	...
	/*
	 * Autoroutes a connection between p_start_set and p_dest_set. Returns
	 ...
	 */
	public AutorouteResult autoroute_connection(Set<Item> p_start_set, Set<Item> p_dest_set, AutorouteControl p_ctrl,
		...
		try {
			...
			LOGGER.error("AutorouteEngine.autoroute_connection: Exception in MazeSearchAlgo.get_instance");
		...
		if (maze_search_algo != null) {
			try {
				...
				LOGGER.error("AutorouteEngine.autoroute_connection: Exception in maze_search_algo.find_connection");
				LOGGER.error(e.getMessage(), e);
			}
		}
		...
		if (search_result != null) {
			try {
				autoroute_result = LocateFoundConnectionAlgo.get_instance(search_result, p_ctrl,
						this.autoroute_search_tree, board.rules.get_trace_angle_restriction(), p_ripped_item_list,
		...
		if (autoroute_result.connection_items == null) {
			if (this.board.get_test_level().ordinal() >= TestLevel.CRITICAL_DEBUGGING_OUTPUT.ordinal()) {
				LOGGER.warn("AutorouteEngine.autoroute_connection: result_items != null expected");
			}
		...
		InsertFoundConnectionAlgo insert_found_connection_algo = InsertFoundConnectionAlgo
				.get_instance(autoroute_result, board, p_ctrl);
	 ...
	 * shape will be a subshape of the start shape, which does not overlap with any
	 * obstacle, and it is as big as possible. p_contained_points will remain
	...
	/**
	 * Returns the first elemment in the list of incomplete expansion rooms or null,
	...
	public void remove_complete_expansion_room(CompleteFreeSpaceExpansionRoom p_room) {
		...
		for (ExpansionDoor curr_door : room_doors) {
			...
			if (curr_neighbour != null) {
				...
				if (intersection.dimension() == 1) {
					...
					IncompleteFreeSpaceExpansionRoom new_incomplete_room = add_incomplete_expansion_room(
							new_incomplete_room_shape, room_layer, intersection);
		...
		if (complete_expansion_rooms != null) {
			...
			LOGGER.info("AutorouteEngine.remove_complete_expansion_room: this.complete_expansion_rooms is null");
		}
	...
	public Collection<CompleteFreeSpaceExpansionRoom> complete_expansion_room(IncompleteFreeSpaceExpansionRoom p_room) {
		...
		try {
			...
			Collection<IncompleteFreeSpaceExpansionRoom> completed_shapes = this.autoroute_search_tree
					.complete_shape(p_room, this.net_no, ignore_object, from_door_shape);
	...
	/**
	 * Completes the shapes of the neigbour rooms of p_room, so that the doors of
	...
	/**
	 * Invalidates all drill pages intersecting with p_shape, so the they must be
	...
	/**
	 * Reset all doors for autorouting the next connnection, in case the autorouting
